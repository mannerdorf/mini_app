...
import { type Route, type Job, routeStartAddress } from "../../../lib/pickup/model";
...
export function DriverHome({ routes, jobs, date, today, loading, onToday, onRoute, stale, pending }: {
  ...
  return <section className="pk-driver-home" aria-label="Обзор дня">
    <div className="pk-driver-day-card">
      ...
      <p>{stale ? "Показаны сохранённые данные" : "Данные последней синхронизации"}{pending ? ` · ${pending} отметок ждут отправки` : ""}</p>
    ...
    <h2>Рейсы на день</h2>
    {!routes.length && <div className="pk-panel"><h3>{loading ? "Загружаем рейсы…" : "Назначенных рейсов пока нет"}</h3><p className="pk-hint">{loading ? "Подождите немного." : "Проверьте город и дату. Новый рейс появится после публикации диспетчером."}</p></div>}
    {routes.map((route) => {
...
export function DriverProfile({ account, route, blocked }: { account: Account; route?: Route; blocked: boolean }) {
  ...
  return <section className="pk-driver-profile" aria-label="Профиль водителя">
    <div className="pk-panel"><p className="pk-eyebrow">ВОДИТЕЛЬ HAULZ</p><h2>{route?.snapshot.driver?.name || account.login}</h2><p className="pk-hint">{account.login}</p>
      {route?.snapshot.driver?.data.phone && <p>{route.snapshot.driver.data.phone}</p>}
      {route?.snapshot.vehicle && <p>Автомобиль рейса: <strong>{route.snapshot.vehicle.name} · {route.snapshot.vehicle.data.plate}</strong></p>}
    </div>
    <div className="pk-panel"><h2>Приложение</h2><p>{version?.platformLabel || "Определяем платформу…"}</p><p>Версия {version?.install.versionName || "—"}{version?.install.buildNumber != null ? ` · сборка ${version.install.buildNumber}` : ""}</p>
      ...
      <button type="button" className="pk-primary pk-driver-mobile-cta" disabled={!version || blocked || checking} onClick={async () => {
        ...
        if (!version.isNativeAndroid) { reloadWebApp(); return; }
      ...
      }}><RefreshCw size={18} />{checking ? "Проверяем…" : version?.isNativeAndroid ? "Проверить обновление" : "Обновить веб-версию"}</button>

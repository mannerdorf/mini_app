...
export const CLAIM_STATUS_LABELS: Record<ClaimStatusKey, string> = {
  draft: "Черновик",
  new: "Новая",
...
export const CLAIM_STATUS_BADGE: Record<ClaimStatusKey, { bg: string; color: string }> = {
...
export function resolveClaimStatusKey(status: string): ClaimStatusKey {
  if (status in CLAIM_STATUS_LABELS) return status as ClaimStatusKey;
  return "new";
}

...
function loadLeaflet(): Promise<LeafletApi> {
  return new Promise((resolve, reject) => {
    ...
    const cssId = "leaflet-css-haulz-docs";
    ...
    const scriptId = "leaflet-js-haulz-docs";
...
export function DocumentsOrderMapPicker({
  open,
  onClose,
  authScope,
  city,
  ...
  useEffect(() => {
    ...
    if (draftAddr?.point && debouncedQuery.trim() === draftAddr.fullAddress.trim()) {
      setSuggestions([]);
      setSuggestLoading(false);
  ...
  }, [open, authScope, debouncedQuery, city, draftAddr?.point, draftAddr?.fullAddress]);
  ...
  const resolveOnMap = async (lat: number, lon: number) => {
    ...
    setMapError(null);
    try {
      const r = await fetchDocumentsGeocode(authScope, { lat, lon, city });
  ...
  const pickSuggestion = async (s: DocumentsSuggestItem) => {
    ...
    try {
      ...
      applyDraft(vague ? s.fullAddress : r.fullAddress, s.label || r.label, r.point, s.uri || s.id);
    } catch (e) {

...
2. У аккаунта должно быть право **`service_mode`** («Служебный режим») — включает администратор.
3. **Профиль → API** → «Создать ключ».
4. **Сохранить полный токен** — он показывается один раз.
...
Требуется **`service_mode`** у пользователя.
...
| 403 | Нет нужного scope; нет `service_mode`; ИНН запрещён для ключа |
...
- [ ] Пользователю включён `service_mode`

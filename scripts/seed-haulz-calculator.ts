...
import type { CityCode, PickupTier } from "../lib/haulzCalculator/types.js";
...
const KGD_SEED_EXITS = [
  { code: "KAD_01", name: "Советский проспект (въезд)", lat: 54.726, lon: 20.456 },
  { code: "KAD_02", name: "Московский проспект", lat: 54.673, lon: 20.508 },
  { code: "KAD_03", name: "Ленинский проспект", lat: 54.691, lon: 20.525 },
  { code: "KAD_04", name: "Приморское кольцо (север)", lat: 54.76, lon: 20.48 },
  { code: "KAD_05", name: "Гурьевск (юг)", lat: 54.768, lon: 20.62 },
  { code: "KAD_06", name: "Зеленоградский район", lat: 54.812, lon: 20.148 },
  { code: "KAD_07", name: "Балтийское шоссе", lat: 54.645, lon: 20.352 },
  { code: "KAD_08", name: "Южный въезд", lat: 54.615, lon: 20.52 },
];
...
async function seedRingCity(
  pool: Awaited<ReturnType<typeof getPool>>,
  cityCode: CityCode,
  exits: { code: string; name: string; lat: number; lon: number }[],
) {
  ...
  const { rows } = await pool.query<{
    ...
    `select id, city_code, code, name, lat::float8 as lat, lon::float8 as lon, active, sort_order
     from haulz_calc_ring_exits where city_code = $1 and active`,
    [cityCode],
  );
...
async function main() {
  ...
  if (mxlPath) {
    ...
    await seedRingCity(pool, "moscow", exits);
    console.log(`MKAD exits seeded: ${exits.length}`);
  }

  await seedRingCity(pool, "kaliningrad", KGD_SEED_EXITS);

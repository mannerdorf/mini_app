...
Если в контексте есть поле cargoList — это список перевозок пользователя (number, status, datePrih, dateVr, stateBill, sum, sender, receiver, customer). Используй эти данные для ответов на вопросы: «мои перевозки», «что в пути», «что доставлено», «сколько не оплачено», «статус по номеру», «перевозка № …» и т.п. Отвечай по факту из cargoList; если списка нет — предложи уточнить или открыть раздел «Грузы».

...
async function handler(req: VercelRequest, res: VercelResponse) {
  const ctx = initRequestContext(req, res, "cargo-timeline-report");
  if (req.method !== "POST") {
  ...
  if (!access) {
    return res.status(403).json({ error: "Доступ только для пользователей HAULZ", request_id: ctx.requestId });
  }

  if (!body.serviceMode) {
    return res.status(403).json({ error: "Отчёт доступен в служебном режиме", request_id: ctx.requestId });
  }
  ...
  try {
    ...
    logError(ctx, "cargo_timeline_report_failed", e);

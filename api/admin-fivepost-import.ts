...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  const ctx = initRequestContext(req, res, "admin_fivepost_import");
  if (req.method !== "POST") {
  ...
  try {
    ...
    const login = String((payload as { login?: string })?.login ?? "admin").trim();
    ...
    await writeAuditLog(pool, {
      action: "fivepost_import",
      target_type: "fivepost_import_batches",
    ...
    logError(ctx, "admin_fivepost_import_failed", e);

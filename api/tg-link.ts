...
async function setRedis(key: string, value: string, ttl: number) {
  ...
  if (!url || !token) {
    console.warn("[tg-link] Upstash Redis not configured");
    return false;
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error("[tg-link] Redis set error:", response.status, text);
      return false;
    }
    ...
    console.error("[tg-link] Redis set exception:", error);
    return false;
  }
}

export default async function handler(req: VercelRequest, res: VercelResponse) {
  const ctx = initRequestContext(req, res, "tg-link");
  if (req.method !== "POST") {
  ...
  try {
    ...
    logError(ctx, "tg_link_failed", error);

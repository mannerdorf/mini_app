...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!department) return res.status(400).json({ error: "Не указано подразделение" });
  if (!logisticsStage) return res.status(400).json({ error: "Не указан этап логистики" });

  const wb = XLSX.read(file.buffer, { type: "buffer" });
  const ws = wb.Sheets[wb.SheetNames[0]];
  const data = XLSX.utils.sheet_to_json(ws, { header: 1, raw: true }) as unknown[][];
  if (!data.length) return res.status(400).json({ error: "Пустой файл" });

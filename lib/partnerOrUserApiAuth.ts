...
import { getBearerPartnerToken, getConfiguredPartnerKeys, verifyPartnerApiKey } from "./partnerApi.js";
...
/**
 * Партнёрский env-ключ или персональный ключ `haulz_…` из БД (scopes + ИНН).
 */
export async function resolvePartnerOrUserApiAuth(
  ...
  const envKeys = getConfiguredPartnerKeys();
  const token = getBearerPartnerToken(req);
  if (!token) {
    sendJson(res, 401, {
      error: "Укажите Authorization: Bearer — партнёрский ключ (env) или персональный ключ haulz_… из профиля",
      request_id: requestId,
    });
    return { ok: false };
  }

  if (envKeys.length > 0 && verifyPartnerApiKey(token)) {
  ...
  if (envKeys.length === 0) {
    sendJson(res, 503, {
      error:
        "Партнёрский API по env не настроен. Создайте персональный ключ в профиле (раздел API) или задайте HAULZ_PARTNER_API_KEY.",
      request_id: requestId,
    });
    return { ok: false };
  }

  sendJson(res, 401, { error: "Неверный Bearer-токен", request_id: requestId });
  return { ok: false };
}
